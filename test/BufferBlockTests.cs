...
namespace System.Threading.Tasks.Dataflow.Tests
{
	public class BufferBlockTests
	{
		...
		public async Task TestOutputAvailableAsyncAfterTryReceiveAll()
		{
			...
			var multipleConcurrentTestsTask = TaskShim.WhenAll(Enumerable.Repeat(0, 1000).Select(_ => generator()));
			var timeoutTask = TaskShim.Delay(2000);
			var completedTask = await TaskShim.WhenAny(multipleConcurrentTestsTask, timeoutTask).ConfigureAwait(false);
