	...
	public class TransformManyBlockTests
	{
		...
		public async Task TestCtor()
		{
			var blocks = new[] {
								...
								new TransformManyBlock<int, int>(i => TaskShim.Run(() => DataflowTestHelpers.ToEnumerable(i)), new ExecutionDataflowBlockOptions { MaxMessagesPerTask = 1 })
						};
		...
		public void TestArgumentExceptions()
		{
			...
			Assert.Throws<ArgumentNullException>(() => new TransformManyBlock<int, int>(i => TaskShim.Run(() => DataflowTestHelpers.ToEnumerable(i)), null));
			...
			var generators = new Func<TransformManyBlock<int, int>>[]
						{
								...
								() => new TransformManyBlock<int, int>(i => TaskShim.Run(() => DataflowTestHelpers.ToEnumerable(i)), new ExecutionDataflowBlockOptions { BoundedCapacity = 10, MaxMessagesPerTask = 1 })
						};
		...
		public void TestPost()
		{
			foreach (bool bounded in DataflowTestHelpers.BooleanValues)
				foreach (var tb in new[] { 
								...
								new TransformManyBlock<int, int>(i => TaskShim.Run(() => DataflowTestHelpers.ToEnumerable(i)), new ExecutionDataflowBlockOptions { BoundedCapacity = bounded ? 1 : -1 })})
				{
		...
		public async Task TestReceives()
		{
			for (int test = 0; test < 2; test++)
			{
				foreach (var tb in new[] { 
										...
										new TransformManyBlock<int, int>(i => TaskShim.Run(() => Enumerable.Repeat(i * 2, 1))) })
				{
					tb.PostRange(0, 5);
		...
		public async Task TestCircularLinking()
		{
			...
			foreach (bool sync in DataflowTestHelpers.BooleanValues)
			{
				...
				TransformManyBlock<int, int> tb = sync ?
						...
						new TransformManyBlock<int, int>(i => TaskShim.Run(() => body(i)));
		...
		public async Task TestCancellationExceptionsIgnored()
		{
			foreach (bool sync in DataflowTestHelpers.BooleanValues)
			{
				...
				TransformManyBlock<int, int> t = sync ?
						new TransformManyBlock<int, int>(body) :
						new TransformManyBlock<int, int>(async i => await TaskShim.Run(() => body(i)));
					...
					foreach (int dop in new[] { 1, Environment.ProcessorCount })
					{
						...
						foreach (IList<int> list in new IList<int>[] { new int[1], new List<int> { 0 }, new Collection<int> { 0 } })
						{
							...
							transform = sync ?
									...
									new TransformManyBlock<int, int>(i => TaskShim.Run(() => body(i)), dbo);
