...
namespace System.Threading.Tasks.Dataflow.Tests
{
	...
	{
		private static WeakReference WrapWeakReference<T>(T state) where T : class
		{
...
#else
			var wro = WrapWeakReference<object>(state);
			DataflowBlock.NullTarget<int>().Completion.ContinueWith(t => { var o = state; });
#endif
			state = null;
...
#else
			var blockReferences = new WeakReference[Count];
#endif
...
#else
				blockReferences[i] = WrapWeakReference<BufferBlock<int>>(b);
#endif
				b = null;
				o = null;
			}
		...
		public async Task TestAsObservableAndAsObserver_BroadcastBackPressure()
		{
			...
			await TaskShim.WhenAll(first, second, third);
			...
			source.Post(2);
			await TaskShim.WhenAll(targets[0].ReceiveAsync(), targets[2].ReceiveAsync());
			...
			await TaskShim.WhenAll(targets[1].ReceiveAsync(), targets[1].ReceiveAsync()); // clear two items from second target
			await TaskShim.WhenAll(from target in targets select target.ReceiveAsync()); // clear third item from all targets

			// Complete the source which should complete all the targets
			source.Complete();
			await source.Completion;
			await TaskShim.WhenAll(from target in targets select target.Completion);
		}
		...
		public async Task TestLinkTo_MaxMessages()
		{
			...
			for (int mode = 0; mode < 3; mode++)
			{
				...
				await TaskShim.WhenAll(target.Completion, otherTarget.Completion);
		...
		public async Task TestReceive_NotYetAvailable()
		{
			...
			var ignored = TaskShim.Run(() => buffer.Post(1));
			Assert.Equal(expected: 1, actual: buffer.Receive());
			ignored = TaskShim.Run(() => buffer.Post(2));
			Assert.Equal(expected: 2, actual: buffer.Receive(new CancellationTokenSource().Token));
			ignored = TaskShim.Run(() => buffer.Post(3));
			Assert.Equal(expected: 3, actual: buffer.Receive(TimeSpan.FromDays(1)));
			ignored = TaskShim.Run(() => buffer.Post(4));
			Assert.Equal(expected: 4, actual: buffer.Receive(TimeSpan.FromDays(1), new CancellationTokenSource().Token));
		...
		public async Task TestReceive_Cancellation()
		{
			...
			{
				var cts = new CancellationTokenSource();
				var ignored = TaskShim.Run(() => cts.Cancel()); // as elsewhere, this test should always succeed, but is racy as to what's being tested
			...
			{
				var cts = new CancellationTokenSource();
				var ignored = TaskShim.Run(() => bb.Post(1)); // as elsewhere, this test should always succeed, but is racy as to what's being tested
