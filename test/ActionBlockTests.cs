				...
				foreach (var option in options)
				{
					...
					foreach (var target in new[] 
												{ 
														...
														new ActionBlock<char>(c => TaskShim.Run(() => result += c), option) // async 
												})
					{
						result = "";
			...
			var blocks = new[]
						{
								...
								new ActionBlock<int>(i => TaskShim.FromResult(0), options)
						};
			...
			for (int trial = 0; trial < 3; trial++)
				foreach (bool singleProducerConstrained in DataflowTestHelpers.BooleanValues)
				{
					...
					switch (trial)
					{
						...
						case 1: ab = new ActionBlock<int>(i => { thrower(); return TaskShim.FromResult(0); }, options); break;
						case 2: ab = new ActionBlock<int>(i => TaskShim.Run(thrower), options); break;
					}
					for (int i = 0; i < 4; i++)
					{
