	...
	public class TransformBlockTests
	{
		...
		public async Task TestCtor()
		{
			var blocks = new[] {
								...
								new TransformBlock<int, string>(i => TaskShim.Run(() => i.ToString()), new ExecutionDataflowBlockOptions { MaxMessagesPerTask = 1 })
						};
			...
			blocks = new[] {
								...
								new TransformBlock<int, string>(i => TaskShim.Run(() => i.ToString()), 
										new ExecutionDataflowBlockOptions { CancellationToken = new CancellationToken(true) })
						};
			foreach (var block in blocks)
			{
		...
		public void TestArgumentExceptions()
		{
			...
			Assert.Throws<ArgumentNullException>(() => new TransformBlock<int, int>(i => TaskShim.Run(() => i), null));
			...
			var generators = new Func<TransformBlock<int, int>>[]
						{
								...
								() => new TransformBlock<int, int>(i => TaskShim.Run(() => i), new ExecutionDataflowBlockOptions { BoundedCapacity = 10, MaxMessagesPerTask = 1 })
						};
		...
		public void TestPost()
		{
			foreach (bool bounded in DataflowTestHelpers.BooleanValues)
				foreach (var tb in new[] { 
								...
								new TransformBlock<int, int>(i => TaskShim.Run(() => i), new ExecutionDataflowBlockOptions { BoundedCapacity = bounded ? 1 : -1 })})
				{
		...
		public async Task TestLinkToOptions()
		{
			...
			foreach (bool append in DataflowTestHelpers.BooleanValues)
				foreach (var tb in new[] { new TransformBlock<int, int>(i => i), new TransformBlock<int, int>(i => TaskShim.Run(() => i)) })
				{
		...
		public async Task TestReceives()
		{
			for (int test = 0; test < 2; test++)
			{
				foreach (var tb in new[] { 
										...
										new TransformBlock<int, int>(i => TaskShim.Run(() => i * 2)) })
				{
					tb.PostRange(0, 5);
		...
		public async Task TestMessagePostponement()
		{
			...
			foreach (int boundedCapacity in new[] { 1, 3 })
			{
				...
				foreach (var tb in new[] { new TransformBlock<int, int>(i => i, options), new TransformBlock<int, int>(i => TaskShim.Run(() => i), options) })
				{
		...
		public async Task TestCount()
		{
			...
			tb.PostRange(1, 11);
			await TaskShim.Run(() => SpinWait.SpinUntil(() => tb.OutputCount == 10));
			for (int i = 10; i > 0; i--)
			{
				int item;
			...
			foreach (bool post in DataflowTestHelpers.BooleanValues)
				foreach (bool sync in DataflowTestHelpers.BooleanValues)
				{
					...
					Func<TransformBlock<int, int>> func = sync ?
							...
							(Func<TransformBlock<int, int>>)(() => new TransformBlock<int, int>(i => TaskShim.Run(() => i * 2)));
		...
		public async Task TestExceptions()
		{
			...
			var tb3 = new TransformBlock<int, int>((Func<int, Task<int>>)(i => TaskShim.Run((Func<int>)(() => { throw new InvalidTimeZoneException(); }))));
		...
		{
			Func<object, Task<object>> f = x => TaskShim.FromResult<object>(x);
			for (int test = 0; test < 2; test++)
			{
