...
#else
				Func<Task<bool>, bool> continuationFunction = antecedent => OutputAvailableAsyncTarget<TOutput>.s_handleCompletion(antecedent, target);
				...
				private Task ProcessItemAsync(TOutput item)
				{
					...
					try
					{
						...
						if (_tempSendAsyncTaskList != null && _tempSendAsyncTaskList.Count > 0)
						{
							...
							Task<bool[]> allSendAsyncTasksConsolidated = TaskShim.WhenAll(_tempSendAsyncTaskList);
