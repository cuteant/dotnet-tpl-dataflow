...
namespace System.Threading.Tasks.Dataflow.Internal
{
	...
	internal static class Common
	{
...
#else
			var reference = state as WeakReference;
		...
		internal static readonly TimeSpan InfiniteTimeSpan = TaskShim.InfiniteTimeSpan;
		...
		{
			return AsyncEnlightenment.AddDenyChildAttach(toInclude);
		}
...
#else
			TaskCreationOptions options = AsyncEnlightenment._CreationDenyChildAttach;
#endif
		...
		private static void PropagateCompletionAsContinuation(Task sourceCompletionTask, IDataflowBlock target)
		{
			...
			sourceCompletionTask.ContinueWith(task => Common.PropagateCompletion(task, target, AsyncExceptionHandler), 
					CancellationToken.None, GetContinuationOptions(), TaskScheduler.Default);
#endif
		}
