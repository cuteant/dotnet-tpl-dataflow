...
#else
				Action<Task> continuationAction = completed => AsyncCompleteProcessMessageWithTask(completed);
				task.ContinueWith(continuationAction, CancellationToken.None, Common.GetContinuationOptions(TaskContinuationOptions.ExecuteSynchronously), TaskScheduler.Default);
#endif
			}
		}
